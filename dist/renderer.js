...
const soqlInput = document.getElementById("soqlQuery");
const apexTemplateInput = document.getElementById("apexTemplate");
const targetOrgSelect = document.getElementById("targetOrg");
const openOrgBtn = document.getElementById("openOrg");
...
const orgLoadingStatus = document.getElementById("orgLoadingStatus");
const refreshOrgsBtn = document.getElementById("refreshOrgs");
const prepareBatchBtn = document.getElementById("prepareBatch");
const openScriptsBtn = document.getElementById("openScripts");
const openResultsBtn = document.getElementById("openResults");
let currentJobName = "";
let isProcessing = false;
let preparedJobName = "";
// Load SFDX orgs on startup
async function loadSfdxOrgs(useStorage = true) {
    ...
    try {
        ...
        const orgs = await window.electronAPI.getSfdxOrgs(useStorage);
        ...
        resultsDiv.innerHTML = "";
        orgLoadingStatus.textContent = `Found ${orgs.length} organization${orgs.length === 1 ? "" : "s"}`;
    }
    catch (error) {
...
openOrgBtn.addEventListener("click", async () => {
    try {
        const selectedOption = targetOrgSelect.options[targetOrgSelect.selectedIndex];
        const targetOrg = selectedOption.value;
        if (targetOrg) {
            openOrgBtn.disabled = true;
            resultsDiv.innerHTML = `<div>Opening org ${targetOrg}...</div>`;
            const result = await window.electronAPI.openOrg(targetOrg);
...
async function loadJobData(jobName) {
    try {
        const job = await window.electronAPI.getJob(jobName);
        if (job) {
            ...
            const orgsLoaded = new Promise((resolve) => {
            ...
            if (job.lastExecuted) {
                resultsDiv.innerHTML += `<div>Last executed: ${new Date(job.lastExecuted).toLocaleString()}</div>`;
            }
        }
    }
    catch (error) {
...
openScriptsBtn.addEventListener("click", async () => {
    if (jobNameInput.value) {
        const opened = await window.electronAPI.openScriptsFolder(jobNameInput.value);
        if (!opened) {
...
openResultsBtn.addEventListener("click", async () => {
    if (jobNameInput.value) {
        const opened = await window.electronAPI.openResultsFolder(jobNameInput.value);
        if (!opened) {
...
prepareBatchBtn.addEventListener("click", async () => {
    try {
        ...
        const result = await window.electronAPI.prepareBatchFiles(jobNameInput.value, soqlInput.value, apexTemplateInput.value, targetOrgSelect.value);
        // Enable buttons
        runBatchBtn.disabled = false;
        openScriptsBtn.disabled = false;
        openResultsBtn.disabled = false;
...
runBatchBtn.addEventListener("click", async () => {
    try {
        ...
        await window.electronAPI.runBatchProcess(preparedJobName, soqlInput.value, apexTemplateInput.value, targetOrgSelect.value);
        isProcessing = false;
...
pauseBatchBtn.addEventListener("click", async () => {
    if (currentJobName && isProcessing) {
        try {
            await window.electronAPI.pauseBatchProcess(currentJobName);
            pauseBatchBtn.disabled = true;
            resumeBatchBtn.disabled = false;
            isProcessing = false;
        }
...
resumeBatchBtn.addEventListener("click", async () => {
    try {
        ...
        resumeBatchBtn.disabled = true;
        pauseBatchBtn.disabled = false;
        await window.electronAPI.resumeBatchProcess(currentJobName, targetOrgSelect.value);
        isProcessing = false;
        pauseBatchBtn.disabled = true;
    }
    catch (error) {
...
window.electronAPI.onProcessUpdate((event, message) => {
